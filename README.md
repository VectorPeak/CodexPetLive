...
这个项目底层基于 PeakDeskSprite / PySide6 桌宠框架，既适合展示 AI 生成角色，也适合制作桌面陪伴应用、验证角色素材、开发桌宠玩法，或者把一个简单的 Pet 包继续扩展成完整的桌面体验
...
如果你只是想先把桌宠跑起来，下载 Windows 发布包，解压后双击 `PeakDeskSprite.exe`。桌宠会出现在桌面上，你可以在设置面板里切换角色、调整大小、修改气泡行为、配置 LLM 服务，或者把它当作一个日常陪伴型桌面应用使用
...
当前最新版本是 [CodexPetLive v0.8.5](https://github.com/VectorPeak/CodexPetLive/releases/tag/v0.8.5)。这是更名前发布的版本，因此 Windows 发布包仍名为 `PeakDeskSprite-v0.8.5-windows-x64.zip`

下载 zip 后解压，双击 `PeakDeskSprite.exe` 启动。发布包 SHA256 为 `613A454DADBED6F0ADD48A1C2ABAE22D552CE2EC9C0CA81BDA59478402B958F9`
...
<table>
  ...
  <tbody>
    <tr><td width="180" nowrap>启动与应用生命周期</td><td><code>PeakDeskSprite/__main__.py</code></td><td>创建 <code>QApplication</code>，加载宠物、通知、附件、系统面板、仪表盘，并通过 Qt Signal 把它们连接起来</td></tr>
    ...
    <tr><td width="180" nowrap>桌面宠物运行时</td><td><code>PeakDeskSprite/PeakDeskSprite.py</code></td><td>主宠物窗口和运行时协调器，负责角色显示、动作切换、状态变化、鼠标交互和信号分发</td></tr>
    ...
    <tr><td width="180" nowrap>角色与素材系统</td><td><code>PeakDeskSprite/conf.py</code></td><td>负责读取角色、动作、物品、存档等配置，是项目的领域配置核心</td></tr>
    <tr><td width="180" nowrap>通知与气泡</td><td><code>PeakDeskSprite/Notification.py</code></td><td>管理弹窗通知、气泡显示、日志转发等</td></tr>
    <tr><td width="180" nowrap>通知与气泡</td><td><code>PeakDeskSprite/bubbleManager.py</code></td><td>管理不同事件触发的对话气泡，例如饱食度、好感度、点击、专注等</td></tr>
    <tr><td width="180" nowrap>通知与气泡</td><td>LLM 气泡接入</td><td>气泡系统可以在特定事件下请求 LLM 生成短文本，再回填到桌宠气泡中</td></tr>
    <tr><td width="180" nowrap>附件与子宠物</td><td><code>PeakDeskSprite/Accessory.py</code></td><td>管理装饰物、掉落物、子宠物、鼠标装饰等窗口对象</td></tr>
    <tr><td width="180" nowrap>附件与子宠物</td><td>子宠物跟随</td><td>子宠物可以跟随主宠物、关闭跟随、回收或独立显示，是“主角色 + 附属对象”的扩展模型</td></tr>
    <tr><td width="180" nowrap>仪表盘功能</td><td><code>PeakDeskSprite/Dashboard/DashboardUI.py</code></td><td>提供状态、背包、商店、每日任务、动画管理几个页签</td></tr>
    ...
    <tr><td width="180" nowrap>系统设置面板</td><td><code>PeakDeskSprite/SpriteSettings/SpriteControlPanel.py</code></td><td>系统设置主窗口，包含设置、聊天、大模型服务、存档、角色、物品 MOD、迷你宠物等页签</td></tr>
    ...
    <tr><td width="180" nowrap>LLM 功能</td><td><code>PeakDeskSprite/llm_client.py</code></td><td>内置 OpenAI、DeepSeek、Ollama、Custom 四类 OpenAI-compatible Provider 预设</td></tr>
    ...
    <tr><td width="180" nowrap>打包与工具</td><td><code>tools/</code></td><td>包含 HatchPet 转换辅助，用于把其他宠物素材转换成 PeakDeskSprite 结构</td></tr>
