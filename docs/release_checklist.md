...
发布前确认应用内版本、README 发布说明和 Git tag 指向同一个版本。当前仓库没有独立 `VERSION` 文件，应用版本源是 `PeakDeskSprite\settings.py` 里的 `VERSION`。

```powershell
$Version = & $PythonPath -c "import pathlib,re; text=pathlib.Path('PeakDeskSprite/settings.py').read_text(encoding='utf-8'); print(re.search(r'^VERSION\\s*=\\s*[\"'']([^\"'']+)[\"'']', text, re.M).group(1))"
git grep -n $Version -- README.md README_EN.md PeakDeskSprite/settings.py
...
```powershell
$env:PEAKDESKSPRITE_CONFIG_DIR = (Resolve-Path .\release-work).Path + '\smoke-config'
$Process = Start-Process -FilePath '.\release-work\dist\PeakDeskSprite\PeakDeskSprite.exe' -PassThru
Start-Sleep -Seconds 10
if ($Process.HasExited) { throw "PeakDeskSprite exited during release smoke with code $($Process.ExitCode)" }
Stop-Process -Id $Process.Id
Remove-Item Env:\PEAKDESKSPRITE_CONFIG_DIR -ErrorAction SilentlyContinue
```
